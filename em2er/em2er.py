...
class em2er_map:
    ...
    mapvars : dict
        Passed to testcalib.DataMapper(num_neighbors=2, max_distance_xy=15, max_distance_z=0.3) for ease of updating this function down the line.
    ...
    def __init__(
        ...
        self.mapvars = mapvars or dict(num_neighbors=2, max_distance_xy=15, max_distance_z=0.3)
	...
	#map EMI data to ERI mesh using select parameters
        soil_mapper = DataMapper(**self.mapvars) 
        emi2mesh, dist = soil_mapper.mapXYZ(self.meshUTM, [self.emi_inv], [''])
